...
void tkacheva_headman::show_on_screen(QPainter& painter,int* x, int* y, int* array_lens) {
    ...
    *(x) += array_lens[0] + dx;
    painter.drawText(*x, *y, QString::fromStdString(last_name));
    *(x) += array_lens[1] + dx;
    painter.drawText(*x, *y, QString::fromStdString(to_string(age)));
    *(x) += array_lens[2] + dx;
    painter.drawText(*x, *y, QString::fromStdString(group_name));
    *(x) += array_lens[3] + dx;
    painter.drawText(*x, *y, QString::fromStdString(email));
    *(y) += metrics.height() + 10;
}
